...
/*   Updated: 2021/02/04 14:26:20 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		create_threads(t_p *p)
{
	...
	while (++i < p->nb_philos)
	{
		if (pthread_create(&p->philos[i]->th_death, NULL, &check_death, p) != 0)
			return (-1);
	}
	if (p->must_eat_nb != -1)
	...
	while (++i < p->nb_philos)
	{
		if (pthread_create(&p->philos[i]->th_eat, NULL, &update_last_eat, p->philos[i]) != 0)
			return (-1);
	}
	return (0);
}
